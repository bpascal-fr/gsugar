function [est,valpos] = Sliding_CP_est_2D(leaders,sizePatch,j1,j2)
% Perform a global DWT on the whole image (after edge cutting)
% and calculate C1 cp for (half) overlapping patches
% REQUIRES a version DxLx2d
%
% INPUT :
% - leaders   : calculated by DCLx2d_HW.m with J2 >= j2
...
for j=1:j2;
    ...
    LLX=leaders(j).valueall; LLXh=LLX; id0=find(LLX(:)==0);id1=find(LLX(:)~=0); LLX(id0)=min(LLX(id1));
    LLX=log(LLX);
    % replace border leaders (inf) with zeros
    infpos=find(~isfinite(LLX)); LLX(infpos)=0; LLXh(infpos)=0; 
    
    if PS>1 % compute local cumulants
        est.H(:,:,j)=log2(conv2(LLXh,   F1,'same'));
        est.C1(:,:,j)=conv2(LLX,   F1,'same');
        est.C2(:,:,j)=conv2(LLX.^2,F1,'same') - est.C1(:,:,j).^2;
        est.C3(:,:,j)=conv2(LLX.^3,F1,'same') - 3*est.C2(:,:,j).*est.C1(:,:,j) - est.C1(:,:,j).^3;

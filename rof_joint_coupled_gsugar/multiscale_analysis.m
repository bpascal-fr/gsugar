...
% estimate the covariance structure of log-leaders. (image)
%
...
function [L,C] = multiscale_analysis(X, JJ, Nwt, gamint)
    ...
    for ii = JJ
        for jj = JJ
            ...
            S{ii,jj} = xcorr2(log2(L.leaders{ii}),log2(L.leaders{jj}))/(N1*N2) - mean(log2(L.leaders{ii}(:)))*mean(log2(L.leaders{jj}(:)));
            % Truncate it
            s_cov = 2^(ii-1)+2^(jj-1);
            S{ii,jj} = S{ii,jj}(N1-s_cov:N1+s_cov, N2-s_cov:N2+s_cov);
        end

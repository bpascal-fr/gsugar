...
% joint functional for fractal texture segmentation (image)
%
% from
...
function [v, h, t, crit, gap]=FISTAj(L_X,prox)
    ...
    if lambda_v < 0 || lambda_h < 0
        ...
        while (gap_c > eps)&&(it<iter)
            ...
            bgh = bgh + gamman*thh;
            bgv = bgv + gamman*thv;
            [tvh, tvv] = prox_L12(bth/gamman,btv/gamman,lambda_v/gamman);
            th = bth - gamman*tvh;
            tv = btv - gamman*tvv;
            [thh, thv] = prox_L12(bgh/gamman,bgv/gamman,lambda_h/gamman);
            gh = bgh - gamman*thh;
            gv = bgv - gamman*thv;
            ...
            [ov,oh] = op_AtAinvDadj(bth-bths,btv-btvs,bgh-bghs,bgv-bgvs,use);
            v = v - ov;
            h = h - oh;
            ...
            [c,~,g] = PDj_gap(v, h, th, tv, gh, gv, L, adj_tv, adj_th, f_adj0, JJ, prox, use);

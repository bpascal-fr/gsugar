function [coef, leaders, nj] = DLPx1dpointloc(data, Nwt, gamint, p, csym, Jmax, WinLoc)
...
if doLeaders
    if doL;
        ...
        for j=1:J
            ...
            tmploc=minmaxfilt1(leaders(j).sans_voisin.value_sbord,WinLoc,'max','same');
            leaders(j).sv.locsupcoef_sb=tmploc;
            leaders(j).sv.locsupcoef=tmploc(leaders(j).nbord(1):leaders(j).nbord(2));
            
            tmploc=minmaxfilt1(leaders(j).value_sbord,WinLoc,'max','same');
            leaders(j).locsupcoef_sb=tmploc;
            leaders(j).locsupcoef=tmploc(leaders(j).nbord(1):leaders(j).nbord(2));
            
            tmploc=minmaxfilt1(DEC(j,:),WinLoc,'max','same');
            coef(j).locsupcoef_sb=tmploc;
            coef(j).locsupcoef=tmploc(leaders(j).nbord(1):leaders(j).nbord(2));
        end
    else
        %% these are the wavelet leaders mimicking an almost symmetric (3)\lambda neighborhood of the size of that of the dyadic leaders
        WL0=zeros(size(wt));

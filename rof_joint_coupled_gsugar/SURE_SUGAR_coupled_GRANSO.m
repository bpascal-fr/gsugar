% Function computing SURE and SUGAR for coupled functional for fractal texture 
...
function [sure_fdmc,sugar,v,h,Eh,dh_lv, dh_lh,Edh_lv,Edh_lh] = SURE_SUGAR_coupled_GRANSO(L, lbd, sure, S)
    ...
    prox.lambda = lbd(1);
    prox.alpha  = lbd(2);
    [v, h, ~, Eh, ~, ~, dh_lv, dh_lh, ~, ~, Edh_lv, Edh_lh]=dPA_PDc(L,prox,sure);
    
    tmp = get_init;
    if isfield(tmp,'h')
        figure(1);
        subplot(236);imagesc(tmp.h); axis off image
